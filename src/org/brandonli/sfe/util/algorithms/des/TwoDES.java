...
public class TwoDES {
	...
	public static byte[] des_attack(String plaintext, int N_BYTE) throws Exception {
		...
		byte[] finalEncryptedText = m_encrypter2.doFinal(encryptedText1);

		BigInteger guess = new BigInteger("0");
		BigInteger N_COMB = new BigInteger("340282366920938463463374607431768211456");
		...
		for (long i = 0; i < Math.pow(2.0, 56); i++) {
			...
			for (long j = 0; j < Math.pow(2.0, 56); j++) {
				...
				try {
					...
					byte[] text_g = m_decrypter2.doFinal(m_decrypter1.doFinal(finalEncryptedText));
					String plain_text = new String(text_g);

					if (plain_text.equals(new String(clearText))) {

						System.out.println("Cracked Both Keys");
	...
	public static void main(String[] args) throws Exception {
		...
		byte[] ris = des_attack(plaintext, 16);
