...
public class CaesarCipher {
	...
	public static void main(String[] args) {
		...
		String code = "qwerty"; // Testing Password
	...
	public static void start(String code) {
		...
		final String[] encryptedContents = new String[100]; // Make an array of encrypted contents

		for (int i = 0; i < encryptedContents.length; i++) { // Encrypt Contents

			final Random r = new Random();
			encryptedContents[i] = encrypt(code, r.nextInt(128)); // Encrypt code many times with random shifts

		}
		
		// Calculating the time it takes for each password to be cracked takes too short, so 
		// we calculate the time it takes for it to break 100 passwords.
		
		final long startTime = System.nanoTime(); // Start time

		for (int i = 0; i < encryptedContents.length; i++) { // Decrypt all passwords in the string array

			final String encryptedMessage = encryptedContents[i];

			decrypt(encryptedMessage, code); // Decrypt
